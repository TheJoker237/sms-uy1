                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="page-sub-header">
                            <h3 class="page-title">Examens List</h3>
                            <ul class="breadcrumb">
                                    ...
                                    <tbody>
                                        @foreach ($sessionList as $key=>$session)
                                            <tr>
                                                ...
                                                <td><a href="{{url('examen/session-list')}}">EXM</a>{{ ++$key }}</td>
                                                <td hidden class="id">{{ $session->id }}</td>
                                                <td class="course {{$session->type == 'Rattrapage' ? 'text-danger' : 'text-info'}}">
                                                    {{-- <a href="{{url('examen/session-list')}}"> --}}
                                                        {{ $session->type }}
                                                    {{-- </a> --}}
                                                </td>
                                                <td class="date">
                                                    <a href="{{url('examen/session-list')}}">
                                                        {{ date('d-m-Y', strtotime($session->examen->date)) }}
                                                    </a>
                                                </td>
                                                <td class="course text-info">{{ $session->examen->status }}</td>
                                                <td class="text-end">
                                                        <a href="{{ url('examen/edit/'.$session->id) }}" class="btn btn-sm bg-danger-light">
